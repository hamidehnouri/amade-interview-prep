...
export default function Stepper({ steps, current, onStepClick, className = "", reachable }: {
  steps: StepItem[]; current: number; onStepClick?: (index: number) => void; className?: string; reachable?: boolean[];
}) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      {steps.map((s, i) => {
        const reached = reachable ? reachable[i] : i <= current;
        ...
        return (
          <div key={s.key} className="flex flex-1 items-center gap-2 last:flex-none">
            <button type="button" disabled={!reached} onClick={() => reached && onStepClick?.(i)} className="flex items-center gap-2 disabled:cursor-default">
              ...
              <span className={`hidden text-[13px] font-medium sm:inline ${i === current ? "text-ink" : reached ? "text-secondary hover:text-ink" : "text-muted"}`}>{s.label}</span>
            </button>
